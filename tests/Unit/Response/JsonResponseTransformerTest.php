...
describe("JsonResponseTransformer", function () {
    ...
    describe("transform method", function () {
        ...
        test("adds single additional header", function () {
            ...
            expect($response->getHeader("X-Custom-Header")[0])->toBe(
                "custom-value",
            );
        });
        ...
        test("adds multiple additional headers", function () {
            ...
            expect($response->getHeader("X-Custom-Header-1")[0])->toBe(
                "value-1",
            );
            expect($response->getHeader("X-Custom-Header-2")[0])->toBe(
                "value-2",
            );
            expect($response->getHeader("X-Custom-Header-3")[0])->toBe(
                "value-3",
            );
        });

        test(
            "preserves Content-Type header when adding additional headers",
            function () {
                $transformer = new JsonResponseTransformer();
    ...
    describe("edge cases", function () {
        test("handles deeply nested arrays", function () {
            ...
            $body = json_decode($response->getBody()->getContents(), true);
            expect($body["level1"]["level2"]["level3"]["level4"])->toBe(
                "deep value",
            );
        });
    ...
    describe("OutputSpec integration", function () {
        test("transforms output from LinkOutputSpec", function () {
            $transformer = new JsonResponseTransformer();
            $spec = new LinkOutputSpec(new TagOutputSpec());
        ...
        test(
            "transforms ActionController response format with single OutputSpec result",
            function () {
                $transformer = new JsonResponseTransformer();
                $spec = new LinkOutputSpec(new TagOutputSpec());
                $link = TestEntityFactory::createLink();
        ...
        test(
            "transforms ActionController response format with array of OutputSpec results",
            function () {
                $transformer = new JsonResponseTransformer();
                $spec = new LinkOutputSpec(new TagOutputSpec());
    ...
    describe("exception handling", function () {
        test(
            "throws ResponseTransformerException when JSON encoding fails",
            function () {
                $transformer = new JsonResponseTransformer();
                ...
                expect(fn() => $transformer->transform($data))->toThrow(
                    ResponseTransformerException::class,
                );
            },
        );

        test(
            "exception contains error message from original exception",
            function () {
                $transformer = new JsonResponseTransformer();
                $data = [];
                $data["self"] = &$data;
                ...
                expect(fn() => $transformer->transform($data))->toThrow(
                    function (ResponseTransformerException $e) {
                        return str_contains(
                            $e->getMessage(),
                            "Generating JSON response failed",
                        );
                    },
                );
            },
        ...
        test("exception preserves original exception as previous", function () {
            ...
            expect(fn() => $transformer->transform($data))->toThrow(function (
                ResponseTransformerException $e,
            ) {
        ...
        test(
            "throws exception when headers parameter causes encoding failure",
            function () {
                $transformer = new JsonResponseTransformer();
                ...
                $response = $transformer->transform($data, 200, [
                    "X-Custom" => "value",
                ]);
                expect($response)->toBeInstanceOf(ResponseInterface::class);
            },
