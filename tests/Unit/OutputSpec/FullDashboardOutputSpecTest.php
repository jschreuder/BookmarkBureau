...
describe('FullDashboardOutputSpec', function () {
    ...
    describe('supports method', function () use ($createSpec) {
        test('supports DashboardWithCategoriesAndFavorites objects', function () use ($createSpec) {
            $spec = $createSpec();
        ...
        test('does not support Dashboard objects', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
        ...
        test('does not support stdClass objects', function () use ($createSpec) {
            $spec = $createSpec();
    ...
    describe('transform method', function () use ($createSpec) {
        test('transforms DashboardWithCategoriesAndFavorites to array with all fields', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $dashboardView = new DashboardWithCategoriesAndFavorites(
                $dashboard,
            ...
            expect($result)->toHaveKeys(['id', 'title', 'description', 'icon', 'created_at', 'updated_at', 'categories', 'favorites']);
        });

        test('includes dashboard fields in result', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard(
        ...
        test('transforms empty categories and favorites', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $dashboardView = new DashboardWithCategoriesAndFavorites(
                $dashboard,
        ...
        test('transforms single category with no links', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
        ...
        test('transforms single category with multiple links', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            ...
            expect($result['categories'][0]['links'][0]['title'])->toBe('Link 1');
            expect($result['categories'][0]['links'][1]['title'])->toBe('Link 2');
            expect($result['categories'][0]['links'][2]['title'])->toBe('Link 3');
        });

        test('transforms multiple categories with links', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
        ...
        test('transforms favorites without categories', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $favorite1 = TestEntityFactory::createLink(title: new Title('Favorite 1'));
            $favorite2 = TestEntityFactory::createLink(title: new Title('Favorite 2'));
        ...
        test('transforms complete dashboard with categories and favorites', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard(
            ...
            $categoryLink1 = TestEntityFactory::createLink(title: new Title('GitHub'));
            $categoryLink2 = TestEntityFactory::createLink(title: new Title('GitLab'));
            ...
            $categoryLink3 = TestEntityFactory::createLink(title: new Title('HN'));

            $favorite1 = TestEntityFactory::createLink(title: new Title('Gmail'));
            $favorite2 = TestEntityFactory::createLink(title: new Title('Calendar'));
            ...
            expect($result['categories'][0]['links'])->toHaveCount(2);
            expect($result['categories'][0]['links'][0]['title'])->toBe('GitHub');
            expect($result['categories'][0]['links'][1]['title'])->toBe('GitLab');

            expect($result['categories'][1]['title'])->toBe('News');
            expect($result['categories'][1]['color'])->toBe('#33FF57');
            expect($result['categories'][1]['links'])->toHaveCount(1);
            expect($result['categories'][1]['links'][0]['title'])->toBe('HN');
        ...
        test('preserves link properties in categories', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $category = TestEntityFactory::createCategory(dashboard: $dashboard);
            $link = TestEntityFactory::createLink(
            ...
            expect($result['categories'][0]['links'][0]['id'])->toBe($link->linkId->toString());
            expect($result['categories'][0]['links'][0]['url'])->toBe('https://example.com');
            expect($result['categories'][0]['links'][0]['title'])->toBe('Example');
            expect($result['categories'][0]['links'][0]['description'])->toBe('Example description');
            expect($result['categories'][0]['links'][0])->toHaveKeys(['id', 'url', 'title', 'description', 'icon', 'created_at', 'updated_at']);
        });

        test('preserves link properties in favorites', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $favorite = TestEntityFactory::createLink(
                url: new \jschreuder\BookmarkBureau\Entity\Value\Url('https://favorite.com'),
            ...
            expect($result['favorites'][0]['id'])->toBe($favorite->linkId->toString());
            expect($result['favorites'][0]['url'])->toBe('https://favorite.com');
            expect($result['favorites'][0]['title'])->toBe('Favorite Link');
            expect($result['favorites'][0]['description'])->toBe('Favorite description');
            expect($result['favorites'][0])->toHaveKeys(['id', 'url', 'title', 'description', 'icon', 'created_at', 'updated_at']);
        });
        ...
        test('preserves category properties', function () use ($createSpec) {
            ...
            expect($result['categories'][0]['id'])->toBe($category->categoryId->toString());
            expect($result['categories'][0]['dashboard_id'])->toBe($dashboard->dashboardId->toString());
            ...
            expect($result['categories'][0])->toHaveKeys(['id', 'dashboard_id', 'title', 'color', 'sort_order', 'created_at', 'updated_at', 'links']);
        });

        test('throws InvalidArgumentException when transforming unsupported object', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
        ...
        test('exception message contains class name and unsupported type', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            ...
            expect(fn() => $spec->transform($dashboard))
                ...
                ->toThrow(function (InvalidArgumentException $e) {
                    return str_contains($e->getMessage(), FullDashboardOutputSpec::class)
    ...
    describe('edge cases', function () use ($createSpec) {
        test('handles large number of categories', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
        ...
        test('handles large number of links in a category', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $category = TestEntityFactory::createCategory(dashboard: $dashboard);
            ...
            $links = [];
            for ($i = 0; $i < 100; $i++) {
                $links[] = TestEntityFactory::createLink(title: new Title("Link $i"));
            }
            ...
            expect($result['categories'][0]['links'][0]['title'])->toBe('Link 0');
            expect($result['categories'][0]['links'][99]['title'])->toBe('Link 99');
        });

        test('handles large number of favorites', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();

            $favorites = [];
            for ($i = 0; $i < 50; $i++) {
                $favorites[] = TestEntityFactory::createLink(title: new Title("Favorite $i"));
            }
        ...
        test('handles categories with mixed link counts', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            ...
            $category3 = TestEntityFactory::createCategory(dashboard: $dashboard);

            $link1 = TestEntityFactory::createLink();
            $link2 = TestEntityFactory::createLink();
            $link3 = TestEntityFactory::createLink();

            $categoryWithLinks1 = new CategoryWithLinks($category1, new LinkCollection());
            $categoryWithLinks2 = new CategoryWithLinks($category2, new LinkCollection($link1, $link2, $link3));
            $categoryWithLinks3 = new CategoryWithLinks($category3, new LinkCollection($link1));

            $dashboardView = new DashboardWithCategoriesAndFavorites(
                $dashboard,
                new CategoryWithLinksCollection($categoryWithLinks1, $categoryWithLinks2, $categoryWithLinks3),
                new LinkCollection()
            );
        ...
        test('handles complex nested structure with all optional fields', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard(
                title: new Title('Complex Dashboard'),
                description: 'A dashboard with all optional fields set',
                icon: new \jschreuder\BookmarkBureau\Entity\Value\Icon('dashboard-icon')
            );
            ...
            $link = TestEntityFactory::createLink(
                url: new \jschreuder\BookmarkBureau\Entity\Value\Url('https://complex.example.com'),
                title: new Title('Complex Link'),
                description: 'A link with all fields',
                icon: new \jschreuder\BookmarkBureau\Entity\Value\Icon('link-icon')
            );

            $favorite = TestEntityFactory::createLink(
                url: new \jschreuder\BookmarkBureau\Entity\Value\Url('https://favorite.example.com'),
                title: new Title('Complex Favorite'),
                description: 'A favorite with all fields',
                icon: new \jschreuder\BookmarkBureau\Entity\Value\Icon('favorite-icon')
            );

            $categoryWithLinks = new CategoryWithLinks($category, new LinkCollection($link));
            ...
            expect($result['categories'][0]['links'][0]['icon'])->toBe('link-icon');
            expect($result['favorites'][0]['icon'])->toBe('favorite-icon');
        });
    });

    describe('integration with OutputSpecInterface', function () use ($createSpec) {
        test('transform method signature matches interface', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $dashboardView = new DashboardWithCategoriesAndFavorites(
                $dashboard,
        ...
        test('can be used polymorphically through interface', function () use ($createSpec) {
            $spec = $createSpec();
            $interface = $spec;
            $dashboard = TestEntityFactory::createDashboard();
            $dashboardView = new DashboardWithCategoriesAndFavorites(
        ...
        test('composes other OutputSpecs correctly', function () use ($createSpec) {
            $spec = $createSpec();
            $dashboard = TestEntityFactory::createDashboard();
            $category = TestEntityFactory::createCategory(dashboard: $dashboard);
            $link = TestEntityFactory::createLink();
            ...
            expect($result['description'])->toBe($expectedDashboard['description']);

            $categoryOutputSpec = new CategoryOutputSpec();
            $expectedCategory = $categoryOutputSpec->transform($category);

            expect($result['categories'][0]['id'])->toBe($expectedCategory['id']);
            expect($result['categories'][0]['title'])->toBe($expectedCategory['title']);

            $linkOutputSpec = new LinkOutputSpec();
            $expectedLink = $linkOutputSpec->transform($link);

            expect($result['categories'][0]['links'][0]['id'])->toBe($expectedLink['id']);
            expect($result['categories'][0]['links'][0]['url'])->toBe($expectedLink['url']);

            $expectedFavorite = $linkOutputSpec->transform($favorite);
            expect($result['favorites'][0]['id'])->toBe($expectedFavorite['id']);
            expect($result['favorites'][0]['url'])->toBe($expectedFavorite['url']);
        });
    });
});
