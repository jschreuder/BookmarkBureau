...
describe('DashboardOutputSpec', function () {
    ...
    describe('transform method', function () {
        test('transforms Dashboard to array with all fields', function () {
            ...
            expect($result)->toHaveKeys(['id', 'title', 'description', 'icon', 'created_at', 'updated_at']);
        });
        ...
        test('returns dashboard description as string', function () {
            $spec = new DashboardOutputSpec();
            $dashboard = TestEntityFactory::createDashboard(description: 'Dashboard Description');
        ...
        test('returns created_at in ATOM format', function () {
            $spec = new DashboardOutputSpec();
            $createdAt = new DateTimeImmutable('2024-01-15 14:30:00', new DateTimeZone('UTC'));
        ...
        test('returns updated_at in ATOM format', function () {
            $spec = new DashboardOutputSpec();
            $updatedAt = new DateTimeImmutable('2024-01-15 15:45:00', new DateTimeZone('UTC'));
        ...
        test('exception message contains class name and unsupported type', function () {
            ...
            expect(fn() => $spec->transform($link))
                ...
                ->toThrow(function (InvalidArgumentException $e) {
                    return str_contains($e->getMessage(), DashboardOutputSpec::class)
    ...
    describe('edge cases', function () {
        test('handles dashboard with long description', function () {
            $spec = new DashboardOutputSpec();
            $longDescription = str_repeat('A very long description text. ', 100);
            $dashboard = TestEntityFactory::createDashboard(description: $longDescription);
        ...
        test('handles dashboard with special characters in description', function () {
            $spec = new DashboardOutputSpec();
            $description = 'Dashboard with "quotes", \'apostrophes\', & ampersand, and unicode: 北京';
