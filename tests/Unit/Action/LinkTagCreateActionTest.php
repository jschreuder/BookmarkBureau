...
describe('LinkTagCreateAction', function () {
    describe('filter method', function () {
        test('filters all fields with whitespace trimmed', function () {
            ...
            $action = new LinkTagCreateAction($tagService, new LinkTagInputSpec());
    ...
    describe('validate method', function () {
        ...
        test('throws validation error for missing tag_name', function () {
            ...
            expect(fn() => $action->validate(['id' => $uuid]))
                ->toThrow(ValidationFailedException::class);
        });
    });

    describe('execute method', function () {
        test('calls tagService.assignTagToLink with correct parameters', function () {
            $tagService = Mockery::mock(TagServiceInterface::class);
            $uuid = Uuid::uuid4();
            $action = new LinkTagCreateAction($tagService, new LinkTagInputSpec());

            $tagService->shouldReceive('assignTagToLink')
                ->once()
