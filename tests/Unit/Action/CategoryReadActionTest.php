...
describe('CategoryReadAction', function () {
    describe('filter method', function () {
        test('trims whitespace from id', function () {
            ...
            $action = new CategoryReadAction($categoryService, $inputSpec, $outputSpec);
    ...
    describe('execute method', function () {
        test('calls getCategory on service with correct UUID', function () {
            ...
            $categoryService->shouldReceive('getCategory')
        ...
        test('returns correct category data structure', function () {
            ...
            $category = TestEntityFactory::createCategory(
                id: $categoryId,
                title: new \jschreuder\BookmarkBureau\Entity\Value\Title('Test Category'),
                color: new \jschreuder\BookmarkBureau\Entity\Value\HexColor('#FF5733'),
                sortOrder: 5
            );
        ...
        test('passes exact UUID to service', function () {
            ...
            $uuidCapture = null;
            $categoryService->shouldReceive('getCategory')
                ->andReturnUsing(function($uuid) use (&$uuidCapture, $category) {
                    $uuidCapture = $uuid;
                    return $category;
                });
        ...
        test('handles category with color', function () {
            ...
            $category = TestEntityFactory::createCategory(
                id: $categoryId,
                color: new \jschreuder\BookmarkBureau\Entity\Value\HexColor('#00FF00')
            );
    ...
    describe('integration scenarios', function () {
        ...
        test('complete data transformation workflow', function () {
            ...
            $category = TestEntityFactory::createCategory(
                id: $categoryId,
                dashboard: $dashboard,
                title: new \jschreuder\BookmarkBureau\Entity\Value\Title('Integration Test Category'),
                color: new \jschreuder\BookmarkBureau\Entity\Value\HexColor('#ABCDEF'),
                sortOrder: 3
            );
