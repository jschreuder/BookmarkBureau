...
describe('UserService', function () {
    describe('createUser method', function () {
        test('creates a new user with email and plaintext password', function () {
            $email = new Email('test@example.com');
            $plainPassword = 'SecurePassword123!';
            $hashedPassword = new HashedPassword(password_hash($plainPassword, PASSWORD_ARGON2ID));

            $userRepository = Mockery::mock(UserRepositoryInterface::class);
            $userRepository->shouldReceive('existsByEmail')->with($email)->andReturn(false);
            $userRepository->shouldReceive('save')->once();

            $passwordHasher = Mockery::mock(PasswordHasherInterface::class);
            $passwordHasher->shouldReceive('hash')->with($plainPassword)->andReturn($hashedPassword);
            ...
            $service = new UserService($userRepository, $passwordHasher, $unitOfWork);

            $result = $service->createUser($email, $plainPassword);

            expect($result)->toBeInstanceOf(\jschreuder\BookmarkBureau\Entity\User::class);
            expect($result->email)->toBe($email);
            expect($result->passwordHash)->toBe($hashedPassword);
            expect($result->totpSecret)->toBeNull();
        });

        test('throws DuplicateEmailException when email already exists', function () {
            $email = new Email('existing@example.com');
            $plainPassword = 'SecurePassword123!';

            $userRepository = Mockery::mock(UserRepositoryInterface::class);
            $userRepository->shouldReceive('existsByEmail')->with($email)->andReturn(true);
            ...
            expect(fn() => $service->createUser($email, $plainPassword))
        ...
        test('hashes the password before storing', function () {
            ...
            $hashedPassword = new HashedPassword('$argon2id$v=19$m=65536,t=4,p=1$...');
    ...
    describe('getUser method', function () {
        ...
        test('throws UserNotFoundException when user does not exist', function () {
            ...
            expect(fn() => $service->getUser($userId))
    ...
    describe('getUserByEmail method', function () {
        test('retrieves a user by email', function () {
            ...
            $userRepository = Mockery::mock(UserRepositoryInterface::class);
            $userRepository->shouldReceive('findByEmail')->with($email)->andReturn($user);
        ...
        test('throws UserNotFoundException when user with email does not exist', function () {
            ...
            expect(fn() => $service->getUserByEmail($email))
                ->toThrow(UserNotFoundException::class);
        });
    });
    ...
    describe('listAllUsers method', function () {
        test('returns empty collection when no users exist', function () {
            $userRepository = Mockery::mock(UserRepositoryInterface::class);
            $userRepository->shouldReceive('findAll')->andReturn(new UserCollection());
        ...
        test('returns collection with all users', function () {
            ...
            $userRepository = Mockery::mock(UserRepositoryInterface::class);
            $userRepository->shouldReceive('findAll')->andReturn($userCollection);
    ...
    describe('deleteUser method', function () {
        ...
        test('throws UserNotFoundException when user does not exist', function () {
            ...
            expect(fn() => $service->deleteUser($userId))
                ->toThrow(UserNotFoundException::class);
        });
    ...
    describe('changePassword method', function () {
        test('updates user password', function () {
            ...
            $newHashedPassword = new HashedPassword(password_hash($newPlainPassword, PASSWORD_ARGON2ID));
            ...
            $passwordHasher = Mockery::mock(PasswordHasherInterface::class);
            $passwordHasher->shouldReceive('hash')->with($newPlainPassword)->andReturn($newHashedPassword);
        ...
        test('throws UserNotFoundException when user does not exist', function () {
            ...
            expect(fn() => $service->changePassword($userId, $newPlainPassword))
                ->toThrow(UserNotFoundException::class);
        });
    ...
    describe('verifyPassword method', function () {
        test('returns true when password is correct', function () {
            ...
            $user = TestEntityFactory::createUser(
                passwordHash: new HashedPassword(password_hash($plainPassword, PASSWORD_ARGON2ID))
            );
        ...
        test('returns false when password is incorrect', function () {
            ...
            $user = TestEntityFactory::createUser(
                passwordHash: new HashedPassword(password_hash($correctPassword, PASSWORD_ARGON2ID))
            );
    ...
    describe('enableTotp method', function () {
        test('generates and enables TOTP secret for user', function () {
            $userId = Uuid::uuid4();
            $user = TestEntityFactory::createUser(id: $userId, totpSecret: null);
        ...
        test('throws UserNotFoundException when user does not exist', function () {
            ...
            expect(fn() => $service->enableTotp($userId))
                ->toThrow(UserNotFoundException::class);
        });
    ...
    describe('disableTotp method', function () {
        test('disables TOTP for user', function () {
            ...
            $user = TestEntityFactory::createUser(id: $userId, totpSecret: $totpSecret);
        ...
        test('throws UserNotFoundException when user does not exist', function () {
            ...
            expect(fn() => $service->disableTotp($userId))
                ->toThrow(UserNotFoundException::class);
        });
