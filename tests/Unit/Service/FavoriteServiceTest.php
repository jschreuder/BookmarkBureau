...
describe('FavoriteService', function () {
    describe('addFavorite method', function () {
        test('adds a link as favorite to a dashboard with correct sort order', function () {
            ...
            $favorite = TestEntityFactory::createFavorite(dashboard: $dashboard, link: $link);

            $favoriteRepository = Mockery::mock(FavoriteRepositoryInterface::class);
            $favoriteRepository->shouldReceive('getMaxSortOrderForDashboardId')
                ->with($dashboardId)
                ->once()
                ->andReturn(2);
            $favoriteRepository->shouldReceive('isFavorite')
                ->with($dashboardId, $linkId)
                ->once()
                ->andReturn(false);
            $favoriteRepository->shouldReceive('addFavorite')
        ...
        test('wraps add favorite in a transaction', function () {
            ...
            $favoriteRepository->shouldReceive('getMaxSortOrderForDashboardId')->andReturn(-1);
            $favoriteRepository->shouldReceive('addFavorite')->andReturn($favorite);
    ...
    describe('removeFavorite method', function () {
        ...
        test('throws FavoriteNotFoundException when favorite does not exist', function () {
            ...
            expect(fn() => $service->removeFavorite($dashboardId, $linkId))
                ->toThrow(FavoriteNotFoundException::class);
        });
    ...
    describe('integration scenarios', function () {
        test('full workflow: add, reorder, and remove favorites', function () {
            ...
            $favorite2 = TestEntityFactory::createFavorite(dashboard: $dashboard, link: $link2);
        ...
        test('multiple favoring workflow with duplicate prevention', function () {
            $dashboardId = Uuid::uuid4();
            $linkId = Uuid::uuid4();
            $dashboard = TestEntityFactory::createDashboard(id: $dashboardId);
            $link = TestEntityFactory::createLink(id: $linkId);
