...
describe("IdInputSpec", function () {
    describe("getAvailableFields method", function () {
        test("returns array containing default id field", function () {
        ...
        test("returns array containing custom id field name", function () {
            $spec = new IdInputSpec("category_id");
            $fields = $spec->getAvailableFields();

            expect($fields)->toBe(["category_id"]);
            expect(count($fields))->toBe(1);
        });
    });
    ...
    describe("filter method", function () {
        ...
        test("filters custom id field name", function () {
            $spec = new IdInputSpec("dashboard_id");
            $uuid = Uuid::uuid4()->toString();
            ...
            expect($filtered["dashboard_id"])->toBe($uuid);
        });

        test("handles missing custom id field with empty string", function () {
            $spec = new IdInputSpec("link_id");
    ...
    describe("validate method", function () {
        ...
        test("validates custom id field name with UUID", function () {
            $spec = new IdInputSpec("category_id");
            $uuid = Uuid::uuid4()->toString();

            try {
                $spec->validate(["category_id" => $uuid]);
                expect(true)->toBeTrue();
            } catch (ValidationFailedException $e) {
                throw $e;
            }
        ...
        test(
            "throws validation error for custom id field when missing",
            function () {
                $spec = new IdInputSpec("dashboard_id");
    ...
    describe("integration scenarios", function () {
        ...
        test(
            "filter handles whitespace, validate ensures UUID format",
            function () {
                $spec = new IdInputSpec();
                $uuid = Uuid::uuid4()->toString();
        ...
        test("custom id field name workflow", function () {
            $spec = new IdInputSpec("link_id");
            $uuid = Uuid::uuid4()->toString();

            $rawData = ["link_id" => "  {$uuid}  "];
            $filtered = $spec->filter($rawData);
