...
describe("Url Value Object", function () {
    describe("valid URLs", function () {
        ...
        test("creates a valid URL with complex query string", function () {
            $url = new Url(
                "https://example.com/search?q=test%20value&filter=type:article&sort=-date",
            );
    ...
    describe("invalid URLs", function () {
        ...
        test("throws exception for plain text", function () {
            expect(fn() => new Url("not a url"))->toThrow(
                InvalidArgumentException::class,
            );
        });
        ...
        test(
            ...
            function () {
                ...
                expect(fn() => new Url($invalidUrl))->toThrow(
                    InvalidArgumentException::class,
                    "Url Value object must get a valid URL, was given: " .
                        $invalidUrl,
                );
            },
        );
    ...
    describe("immutability", function () {
        test("URL value object is immutable", function () {
            ...
            expect(fn() => ($url->value = "https://different.com"))->toThrow(
                Error::class,
            );
        });
    });
    ...
    describe("equals method", function () {
        test("equals returns true for same URL value", function () {
            $url1 = new Url("https://example.com/path");
            $url2 = new Url("https://example.com/path");

            expect($url1->equals($url2))->toBeTrue();
        });

        test("equals returns false for different URL values", function () {
        ...
        test(
            ...
            function () {
                ...
                expect($url->equals($stdObject))->toBeFalse();
            },
        );
        ...
        test(
            ...
            function () {
                ...
                expect($url->equals($email))->toBeFalse();
            },
        );

        test(
            "equals comparison is case-sensitive for path and query",
            function () {
                $url1 = new Url("https://example.com/Path");
                $url2 = new Url("https://example.com/path");
