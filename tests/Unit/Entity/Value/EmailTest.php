...
describe("Email Value Object", function () {
    describe("valid email addresses", function () {
        ...
        test(
            "creates a valid email with underscore in local part",
            function () {
    ...
    describe("invalid email addresses", function () {
        ...
        test("throws exception for missing @", function () {
            expect(fn() => new Email("userexample.com"))->toThrow(
                InvalidArgumentException::class,
            );
        });

        test("throws exception for missing local part", function () {
            expect(fn() => new Email("@example.com"))->toThrow(
                InvalidArgumentException::class,
            );
        });

        test("throws exception for missing domain", function () {
            expect(fn() => new Email("user@"))->toThrow(
                InvalidArgumentException::class,
            );
        });

        test("throws exception for missing TLD", function () {
            expect(fn() => new Email("user@example"))->toThrow(
                InvalidArgumentException::class,
            );
        });

        test("throws exception for spaces", function () {
            expect(fn() => new Email("user name@example.com"))->toThrow(
                InvalidArgumentException::class,
            );
        });

        test("throws exception for multiple @ symbols", function () {
            expect(fn() => new Email("user@@example.com"))->toThrow(
                InvalidArgumentException::class,
            );
        });
        ...
        test("throws exception with descriptive error message", function () {
            expect(fn() => new Email("invalid"))->toThrow(
                InvalidArgumentException::class,
                "Email Value object must get a valid e-mail address, was given: invalid",
            );
        });
    });
    ...
    describe("immutability", function () {
        test("Email value object is immutable", function () {
            ...
            expect(fn() => ($email->value = "other@example.com"))->toThrow(
                Error::class,
            );
        });
    });
    ...
    describe("equals method", function () {
        test("equals returns true for same email value", function () {
        ...
        test("equals returns false for different email values", function () {
            $email1 = new Email("user@example.com");
        ...
        test(
            ...
            function () {
                ...
                expect($email->equals($stdObject))->toBeFalse();
            },
        );
