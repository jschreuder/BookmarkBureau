...
describe('PdoDashboardRepository', function () {
    ...
    describe('findById', function () {
        test('finds and returns a dashboard by ID', function () {
            ...
            expect($found->dashboardId->toString())->toBe($dashboard->dashboardId->toString());
        ...
        test('correctly maps icon field when present', function () {
            ...
            $dashboardWithIcon = TestEntityFactory::createDashboard(icon: $icon);
    ...
    describe('findAll', function () {
        test('returns all dashboards ordered by title ascending', function () {
            ...
            $dashboard1 = TestEntityFactory::createDashboard(title: new Title('Work'));
            $dashboard2 = TestEntityFactory::createDashboard(title: new Title('Personal'));
            $dashboard3 = TestEntityFactory::createDashboard(title: new Title('Archive'));
            ...
            expect($dashboards[0]->dashboardId->toString())->toBe($dashboard3->dashboardId->toString());
            expect($dashboards[1]->dashboardId->toString())->toBe($dashboard2->dashboardId->toString());
            expect($dashboards[2]->dashboardId->toString())->toBe($dashboard1->dashboardId->toString());
        });
        ...
        test('returns dashboards with all fields populated correctly', function () {
            $pdo = createDashboardDatabase();
            $repo = new PdoDashboardRepository($pdo);
            ...
            expect($dashboards[0]->createdAt->format('Y-m-d H:i:s'))->toBe('2024-01-15 10:00:00');
            expect($dashboards[0]->updatedAt->format('Y-m-d H:i:s'))->toBe('2024-01-15 12:00:00');
        });
    });
    ...
    describe('save', function () {
        ...
        test('preserves timestamps on insert', function () {
            ...
            $dashboard = TestEntityFactory::createDashboard(createdAt: $createdAt, updatedAt: $updatedAt);
        ...
        test('saves dashboards with icon', function () {
            $pdo = createDashboardDatabase();
            $repo = new PdoDashboardRepository($pdo);
            $icon = new \jschreuder\BookmarkBureau\Entity\Value\Icon('custom-icon');
        ...
        test('updates icon field when saving existing dashboard', function () {
            ...
            $newIcon = new \jschreuder\BookmarkBureau\Entity\Value\Icon('updated-icon');
        ...
        test('can nullify icon when updating dashboard', function () {
            $pdo = createDashboardDatabase();
            $repo = new PdoDashboardRepository($pdo);
            $icon = new \jschreuder\BookmarkBureau\Entity\Value\Icon('initial-icon');
            $dashboard = TestEntityFactory::createDashboard(icon: $icon);
    ...
    describe('delete', function () {
        test('deletes a dashboard by ID', function () {
            ...
            expect(fn() => $repo->findById($dashboard->dashboardId))
                ->toThrow(DashboardNotFoundException::class);
        });
        ...
        test('cascades delete to categories', function () {
            ...
            $pdo->prepare('INSERT INTO categories (category_id, dashboard_id, title) VALUES (?, ?, ?)')
                ->execute([$categoryId->getBytes(), $dashboard->dashboardId->getBytes(), 'Test Category']);

            // Verify the category was inserted
            $checkStmt = $pdo->prepare('SELECT COUNT(*) as count FROM categories WHERE dashboard_id = ?');
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) as count FROM categories WHERE dashboard_id = ?');
        ...
        test('cascades delete to favorites', function () {
            ...
            // Insert a link first
            $pdo->prepare('INSERT INTO links (link_id, url, title, description) VALUES (?, ?, ?, ?)')
                ->execute([$linkId->getBytes(), 'https://example.com', 'Example', 'Test link']);

            // Insert a favorite
            $pdo->prepare('INSERT INTO favorites (dashboard_id, link_id) VALUES (?, ?)')
                ->execute([$dashboard->dashboardId->getBytes(), $linkId->getBytes()]);

            // Verify the favorite was inserted
            $checkStmt = $pdo->prepare('SELECT COUNT(*) as count FROM favorites WHERE dashboard_id = ?');
            $checkStmt->execute([$dashboard->dashboardId->getBytes()]);
            $beforeDelete = $checkStmt->fetch(PDO::FETCH_ASSOC);
            expect($beforeDelete['count'])->toBe(1);
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) as count FROM favorites WHERE dashboard_id = ?');
            $stmt->execute([$dashboard->dashboardId->getBytes()]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            expect($result['count'])->toBe(0);
        });

        test('cascade delete correctly removes nested cascade data', function () {
            $pdo = createDashboardDatabase();
            $repo = new PdoDashboardRepository($pdo);
            $dashboard = TestEntityFactory::createDashboard();
            $categoryId = Uuid::uuid4();
