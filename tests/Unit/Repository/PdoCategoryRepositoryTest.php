...
describe('PdoCategoryRepository', function () {
    ...
    function createCategoryRepositories(PDO $pdo) {
        return [
            ...
            new PdoCategoryRepository($pdo, new PdoDashboardRepository($pdo), new PdoLinkRepository($pdo)),
        ];
    }
    ...
    describe('findById', function () {
        test('finds and returns a category by ID', function () {
            ...
            expect($found->categoryId->toString())->toBe($category->categoryId->toString());
        ...
        test('correctly maps color field when present', function () {
            ...
            $category = TestEntityFactory::createCategory(dashboard: $dashboard, color: $color);
    ...
    describe('findByDashboardId', function () {
        test('returns all categories for a dashboard ordered by sort_order', function () {
            $pdo = createCategoryDatabase();
            [$dashboardRepo, $linkRepo, $repo] = createCategoryRepositories($pdo);
            $dashboard = TestEntityFactory::createDashboard();
            ...
            $category1 = TestEntityFactory::createCategory(dashboard: $dashboard, title: new Title('First'), sortOrder: 1);
            $category2 = TestEntityFactory::createCategory(dashboard: $dashboard, title: new Title('Second'), sortOrder: 0);
            $category3 = TestEntityFactory::createCategory(dashboard: $dashboard, title: new Title('Third'), sortOrder: 2);
            ...
            expect($categories[0]->categoryId->toString())->toBe($category2->categoryId->toString());
            expect($categories[1]->categoryId->toString())->toBe($category1->categoryId->toString());
            expect($categories[2]->categoryId->toString())->toBe($category3->categoryId->toString());
        });

        test('returns empty collection when dashboard has no categories', function () {
            $pdo = createCategoryDatabase();
            [$dashboardRepo, $linkRepo, $repo] = createCategoryRepositories($pdo);
            $dashboard = TestEntityFactory::createDashboard();
    ...
    describe('save', function () {
        ...
        test('preserves timestamps on insert', function () {
            ...
            $category = TestEntityFactory::createCategory(dashboard: $dashboard, createdAt: $createdAt, updatedAt: $updatedAt);

            insertTestDashboardForCategory($pdo, $dashboard);
            $repo->save($category);

            $found = $repo->findById($category->categoryId);
            expect($found->createdAt->format('Y-m-d H:i:s'))->toBe('2024-01-01 10:00:00');
            expect($found->updatedAt->format('Y-m-d H:i:s'))->toBe('2024-01-01 12:00:00');
        });
        ...
        test('saves categories with null color', function () {
            ...
            [$dashboardRepo, $linkRepo, $repo] = createCategoryRepositories($pdo);
            $dashboard = TestEntityFactory::createDashboard();
            $category = TestEntityFactory::createCategory(dashboard: $dashboard, color: null);
    ...
    describe('delete', function () {
        test('deletes a category by ID', function () {
            ...
            expect(fn() => $repo->findById($category->categoryId))
                ->toThrow(CategoryNotFoundException::class);
        });
        ...
        test('cascades delete to category_links', function () {
            ...
            $pdo->prepare('INSERT INTO category_links (category_id, link_id) VALUES (?, ?)')
                ->execute([$category->categoryId->getBytes(), $link->linkId->getBytes()]);

            // Verify the link was inserted
            $checkStmt = $pdo->prepare('SELECT COUNT(*) as count FROM category_links WHERE category_id = ?');
            ...
            $stmt = $pdo->prepare('SELECT COUNT(*) as count FROM category_links WHERE category_id = ?');
    ...
    describe('addLink', function () {
        test('adds a link to a category with specified sort order', function () {
            $pdo = createCategoryDatabase();
            [$dashboardRepo, $linkRepo, $repo] = createCategoryRepositories($pdo);
            $dashboard = TestEntityFactory::createDashboard();
            $category = TestEntityFactory::createCategory(dashboard: $dashboard);
            ...
            $categoryLink = $repo->addLink($category->categoryId, $link->linkId, 5);

            expect($categoryLink->category->categoryId->toString())->toBe($category->categoryId->toString());
            expect($categoryLink->link->linkId->toString())->toBe($link->linkId->toString());
        ...
        test('throws CategoryNotFoundException when category does not exist', function () {
            ...
            expect(fn() => $repo->addLink($nonExistentCategoryId, $link->linkId, 0))
                ->toThrow(CategoryNotFoundException::class);
        });
    ...
    describe('removeLink', function () {
        test('removes a link from a category', function () {
            ...
            expect($repo->hasLink($category->categoryId, $link->linkId))->toBeFalse();
        });
        ...
        test('throws CategoryNotFoundException when category does not exist', function () {
            ...
            expect(fn() => $repo->removeLink($nonExistentCategoryId, $link->linkId))
                ->toThrow(CategoryNotFoundException::class);
        });
        ...
        test('throws LinkNotFoundException when link does not exist', function () {
            ...
            expect(fn() => $repo->removeLink($category->categoryId, $nonExistentLinkId))
                ->toThrow(LinkNotFoundException::class);
        });

        test('throws LinkNotFoundException when link is not in category', function () {
            $pdo = createCategoryDatabase();
            [$dashboardRepo, $linkRepo, $repo] = createCategoryRepositories($pdo);
            $dashboard = TestEntityFactory::createDashboard();
            $category = TestEntityFactory::createCategory(dashboard: $dashboard);
            ...
            expect(fn() => $repo->removeLink($category->categoryId, $link->linkId))
                ->toThrow(LinkNotFoundException::class);
        });
    });
    ...
    describe('hasLink', function () {
        test('returns true when link is in category', function () {
            ...
            expect($repo->hasLink($category->categoryId, $link->linkId))->toBeTrue();
        });
    ...
    describe('updateLinkSortOrder', function () {
        test('updates sort order for a link in a category', function () {
            ...
            $repo->updateLinkSortOrder($category->categoryId, $link->linkId, 10);

            $stmt = $pdo->prepare('SELECT sort_order FROM category_links WHERE category_id = ? AND link_id = ?');
            $stmt->execute([$category->categoryId->getBytes(), $link->linkId->getBytes()]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        test('throws CategoryNotFoundException when category does not exist', function () {
            ...
            expect(fn() => $repo->updateLinkSortOrder($nonExistentCategoryId, $link->linkId, 0))
                ->toThrow(CategoryNotFoundException::class);
        });
        ...
        test('throws LinkNotFoundException when link does not exist', function () {
            ...
            expect(fn() => $repo->updateLinkSortOrder($category->categoryId, $nonExistentLinkId, 0))
                ->toThrow(LinkNotFoundException::class);
        });
    });
    ...
    describe('reorderLinks', function () {
        test('reorders links in a category using LinkCollection', function () {
            ...
            $repo->reorderLinks($category->categoryId, new LinkCollection($link2, $link1, $link3));

            $stmt = $pdo->prepare('SELECT sort_order FROM category_links WHERE category_id = ? AND link_id = ? ORDER BY sort_order');

            $stmt->execute([$category->categoryId->getBytes(), $link2->linkId->getBytes()]);
            expect($stmt->fetch(PDO::FETCH_ASSOC)['sort_order'])->toBe(0);

            $stmt->execute([$category->categoryId->getBytes(), $link1->linkId->getBytes()]);
            expect($stmt->fetch(PDO::FETCH_ASSOC)['sort_order'])->toBe(1);

            $stmt->execute([$category->categoryId->getBytes(), $link3->linkId->getBytes()]);
        ...
        test('throws CategoryNotFoundException when category does not exist', function () {
            ...
            expect(fn() => $repo->reorderLinks($nonExistentCategoryId, new LinkCollection($link)))
                ->toThrow(CategoryNotFoundException::class);
        });
    });

    describe('getMaxSortOrderForDashboardId', function () {
        test('returns highest sort_order for categories in a dashboard', function () {
            $pdo = createCategoryDatabase();
            [$dashboardRepo, $linkRepo, $repo] = createCategoryRepositories($pdo);
            $dashboard = TestEntityFactory::createDashboard();
            ...
            $category1 = TestEntityFactory::createCategory(dashboard: $dashboard, sortOrder: 2);
            $category2 = TestEntityFactory::createCategory(dashboard: $dashboard, sortOrder: 5);
            $category3 = TestEntityFactory::createCategory(dashboard: $dashboard, sortOrder: 1);

            insertTestCategoryEntity($pdo, $category1);
            insertTestCategoryEntity($pdo, $category2);
            insertTestCategoryEntity($pdo, $category3);

            expect($repo->getMaxSortOrderForDashboardId($dashboard->dashboardId))->toBe(5);
        });
        ...
        test('returns -1 when dashboard has no categories', function () {
            ...
            expect($repo->getMaxSortOrderForDashboardId($dashboard->dashboardId))->toBe(-1);
        });
    });
    ...
    describe('getMaxSortOrderForCategoryId', function () {
        test('returns highest sort_order for links in a category', function () {
            ...
            $pdo->prepare('INSERT INTO category_links (category_id, link_id, sort_order) VALUES (?, ?, ?)')
                ->execute([$category->categoryId->getBytes(), $link1->linkId->getBytes(), 3]);
            $pdo->prepare('INSERT INTO category_links (category_id, link_id, sort_order) VALUES (?, ?, ?)')
                ->execute([$category->categoryId->getBytes(), $link2->linkId->getBytes(), 7]);
            $pdo->prepare('INSERT INTO category_links (category_id, link_id, sort_order) VALUES (?, ?, ?)')
                ->execute([$category->categoryId->getBytes(), $link3->linkId->getBytes(), 1]);

            expect($repo->getMaxSortOrderForCategoryId($category->categoryId))->toBe(7);
        });
        ...
        test('returns -1 when category has no links', function () {
            ...
            expect($repo->getMaxSortOrderForCategoryId($category->categoryId))->toBe(-1);
        });
    });
    ...
    describe('count', function () {
        test('returns the total number of categories', function () {
            ...
            $repo->save(TestEntityFactory::createCategory(dashboard: $dashboard));
            $repo->save(TestEntityFactory::createCategory(dashboard: $dashboard));
            $repo->save(TestEntityFactory::createCategory(dashboard: $dashboard));
    ...
    describe('findCategoryLinksForCategoryId', function () {
        test('returns CategoryLink entities for a category ordered by sort_order', function () {
            $pdo = createCategoryDatabase();
            [$dashboardRepo, $linkRepo, $repo] = createCategoryRepositories($pdo);
            $dashboard = TestEntityFactory::createDashboard();
            $category = TestEntityFactory::createCategory(dashboard: $dashboard);
            ...
            $collection = $repo->findCategoryLinksForCategoryId($category->categoryId);

            expect($collection)->toHaveCount(3);
            $categoryLinks = iterator_to_array($collection);
            expect($categoryLinks[0]->link->linkId->toString())->toBe($link2->linkId->toString());
            expect($categoryLinks[0]->sortOrder)->toBe(0);
            expect($categoryLinks[1]->link->linkId->toString())->toBe($link3->linkId->toString());
            expect($categoryLinks[1]->sortOrder)->toBe(1);
            expect($categoryLinks[2]->link->linkId->toString())->toBe($link1->linkId->toString());
