...
describe('ResourceRouteBuilder', function () {
    describe('initialization', function () {
        test('creates builder with router, resource name, and path segment', function () {
    ...
    describe('registerRead', function () {
        ...
        test('passes closure to router that creates action on invocation', function () {
            $router = Mockery::mock(RouterInterface::class);
            $capturedController = null;

            $router->shouldReceive('get')
                ->with('item-read', '/item/:id', Mockery::on(function($controller) use (&$capturedController) {
            ...
            expect($createdController)->toBeInstanceOf(ControllerInterface::class);
    ...
    describe('registerCreate', function () {
        ...
        test('defers action instantiation until controller is invoked', function () {
            $router = Mockery::mock(RouterInterface::class);
            $capturedController = null;

            $router->shouldReceive('post')
                ->with('item-create', '/item', Mockery::on(function($controller) use (&$capturedController) {
                    $capturedController = $controller;
                    return true;
                }))
                ->once()
    ...
    describe('registerUpdate', function () {
        ...
        test('defers action instantiation until controller is invoked', function () {
            ...
            $router->shouldReceive('put')
                ->with('item-update', '/item/:id', Mockery::on(function($controller) use (&$capturedController) {
                    $capturedController = $controller;
                    return true;
                }))
                ->once()
    ...
    describe('registerDelete', function () {
        ...
        test('defers action instantiation until controller is invoked', function () {
            ...
            $router->shouldReceive('delete')
                ->with('item-delete', '/item/:id', Mockery::on(function($controller) use (&$capturedController) {
                    $capturedController = $controller;
                    return true;
                }))
                ->once()
    ...
    describe('registerCustom', function () {
        test('registers custom GET route', function () {
            ...
            $result = $builder->registerCustom('GET', 'list', '/list', fn() => $action);
        ...
        test('registers custom POST route', function () {
            ...
            $result = $builder->registerCustom('POST', 'bulk', '/bulk', fn() => $action);
        ...
        test('registers custom PUT route', function () {
            ...
            $result = $builder->registerCustom('PUT', 'sync', '/sync', fn() => $action);
        ...
        test('registers custom DELETE route', function () {
            ...
            $result = $builder->registerCustom('DELETE', 'purge', '/purge', fn() => $action);
        ...
        test('registers custom PATCH route', function () {
            ...
            $result = $builder->registerCustom('PATCH', 'partial', '/partial', fn() => $action);

            expect($result)->toBe($builder);
        });

        test('uses resource name as route name when suffix is empty', function () {
            $router = Mockery::mock(RouterInterface::class);
            $router->shouldReceive('get')
        ...
        test('appends path suffix to resource path', function () {
            ...
            $builder->registerCustom('GET', 'search', '/search', fn() => $action);
        });
        ...
        test('throws exception for unsupported HTTP method', function () {
            ...
            expect(fn() => $builder->registerCustom('HEAD', 'test', '/test', fn() => $action))
                ->toThrow(InvalidArgumentException::class, 'Unsupported HTTP method: HEAD');
        });
        ...
        test('returns self for method chaining', function () {
            ...
            $result = $builder->registerCustom('POST', 'custom', '/custom', fn() => $action);
        ...
        test('defers action instantiation until controller is invoked', function () {
            ...
            $router->shouldReceive('post')
                ->with('item-action', '/item/action', Mockery::on(function($controller) use (&$capturedController) {
                    $capturedController = $controller;
                    return true;
                }))
                ->once()
            ...
            $builder = new ResourceRouteBuilder($router, 'item', '/item');
            $builder->registerCustom('POST', 'action', '/action', $actionFactory);
    ...
    describe('lazy loading behavior', function () {
        test('action factory not called during route registration', function () {
            $router = Mockery::mock(RouterInterface::class);
            $router->shouldReceive('get')->andReturn($router);
        ...
        test('action factory called when controller is invoked', function () {
            ...
            $capturedController = null;
            $router->shouldReceive('get')
                ->with('product-read', '/product/:id', Mockery::on(function($controller) use (&$capturedController) {
                    $capturedController = $controller;
                    return true;
                }))
                ->andReturn($router);
        ...
        test('action factory called once per controller invocation', function () {
            $router = Mockery::mock(RouterInterface::class);
            $capturedController = null;
            $router->shouldReceive('get')
                ->with('product-read', '/product/:id', Mockery::on(function($controller) use (&$capturedController) {
        ...
        test('independent lazy loading for each registered route', function () {
            ...
            $router->shouldReceive('get')
                ->with('product-read', '/product/:id', Mockery::on(function($controller) use (&$readController) {
            ...
            $router->shouldReceive('post')
                ->with('product-create', '/product', Mockery::on(function($controller) use (&$createController) {
    ...
    describe('routing configuration', function () {
        ...
        test('custom route with nested path segment', function () {
            $router = Mockery::mock(RouterInterface::class);
            $router->shouldReceive('get')
                ->with('dashboard-favorite', '/dashboard/:id/favorites', Mockery::any())
                ->once()
                ->andReturn($router);

            $action = Mockery::mock(ActionInterface::class);
            $builder = new ResourceRouteBuilder($router, 'dashboard', '/dashboard/:id');
            $builder->registerCustom('GET', 'favorite', '/favorites', fn() => $action);
        });
    });

    describe('action controller creation', function () {
        test('controller wraps action with JsonResponseTransformer', function () {
            $router = Mockery::mock(RouterInterface::class);
            $capturedController = null;
            $router->shouldReceive('post')
                ->with('product-create', '/product', Mockery::on(function($controller) use (&$capturedController) {
                    $capturedController = $controller;
                    return true;
                }))
                ->andReturn($router);
