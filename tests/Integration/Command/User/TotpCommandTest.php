...
describe('TotpCommand', function () {
    describe('execute', function () {
        test('should enable TOTP for user', function () {
            ...
            $user = TestEntityFactory::createUser(email: new \jschreuder\BookmarkBureau\Entity\Value\Email($email));
            $totpSecret = new \jschreuder\BookmarkBureau\Entity\Value\TotpSecret('JBSWY3DPEBLW64TMMQ');

            $userService->shouldReceive('getUserByEmail')
                ->with(Mockery::on(fn($e) => (string)$e === $email))
                ->andReturn($user);

            $userService->shouldReceive('enableTotp')
                ->with($user->userId)
                ->andReturn($totpSecret);

            $tester = new CommandTester($command);
            $statusCode = $tester->execute(['action' => 'enable', 'email' => $email]);
        ...
        test('should disable TOTP for user', function () {
            ...
            $userService->shouldReceive('disableTotp')
                ->with($user->userId)
                ->once();

            $tester = new CommandTester($command);
            $statusCode = $tester->execute(['action' => 'disable', 'email' => $email]);
        ...
        test('should reject invalid action', function () {
            ...
            $tester = new CommandTester($command);
            $statusCode = $tester->execute(['action' => 'invalid', 'email' => 'test@example.com']);
        ...
        test('should show error for non-existent user', function () {
            ...
            $userService->shouldReceive('getUserByEmail')
                ->andThrow(new \jschreuder\BookmarkBureau\Exception\UserNotFoundException());
        ...
        test('should reject invalid email', function () {
            ...
            $tester = new CommandTester($command);
            $statusCode = $tester->execute(['action' => 'enable', 'email' => 'invalid-email']);
