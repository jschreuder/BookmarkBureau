...
describe('ListCommand', function () {
    describe('execute', function () {
        test('should display no users message when empty', function () {
            ...
            $userService->shouldReceive('listAllUsers')
        ...
        test('should display users in table format', function () {
            ...
            $user1 = TestEntityFactory::createUser(email: new \jschreuder\BookmarkBureau\Entity\Value\Email('user1@example.com'));
            $user2 = TestEntityFactory::createUser(email: new \jschreuder\BookmarkBureau\Entity\Value\Email('user2@example.com'));
        ...
        test('should indicate TOTP status correctly', function () {
            ...
            $userWithTotp = TestEntityFactory::createUser(
                email: new \jschreuder\BookmarkBureau\Entity\Value\Email('with-totp@example.com'),
            ...
            $userWithoutTotp = TestEntityFactory::createUser(
                email: new \jschreuder\BookmarkBureau\Entity\Value\Email('without-totp@example.com')
            );

            $userService->shouldReceive('listAllUsers')
                ->andReturn(new UserCollection($userWithTotp, $userWithoutTotp));
