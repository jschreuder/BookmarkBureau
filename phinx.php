...
if (!preg_match("#^(?P<type>[a-z]+):#i", $dsn, $typeMatch)) {
    throw new RuntimeException(
        "Invalid DSN format: unable to determine database type",
    );
}
...
switch ($type) {
    ...
    case "pgsql":
        ...
        if (
            ...
            throw new RuntimeException(
                "Invalid $type DSN format: host not found",
            );
        }
        break;
    ...
    default:
        throw new RuntimeException(
            "Unsupported database type: $type. Supported types: mysql, pgsql, sqlite",
        );
}
