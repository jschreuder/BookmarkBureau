...
final readonly class PdoCategoryRepository implements CategoryRepositoryInterface
{
    ...
    public function findCategoryLinksForCategoryId(UuidInterface $categoryId): CategoryLinkCollection
    {
        ...
        while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
            ...
            $categoryLinks[] = new CategoryLink(
                ...
                createdAt: new DateTimeImmutable($row['category_link_created_at'])
            );
        }
    ...
    public function save(Category $category): void
    {
        ...
        $check = $this->pdo->prepare('SELECT 1 FROM categories WHERE category_id = :category_id LIMIT 1');
        ...
        if ($check->fetch() === false) {
            ...
            try {
                ...
                if (str_contains($e->getMessage(), 'FOREIGN KEY constraint failed') ||
                    ...
                    throw new DashboardNotFoundException('Dashboard not found: ' . $category->dashboard->dashboardId->toString());
                }
                throw $e;
            }
        } else {
    ...
    public function delete(Category $category): void
    {
        ...
        $statement = $this->pdo->prepare('DELETE FROM categories WHERE category_id = :category_id');
        $statement->execute([':category_id' => $category->categoryId->getBytes()]);
    }
